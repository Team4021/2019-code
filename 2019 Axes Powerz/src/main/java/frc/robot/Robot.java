 ...
 // } This guy was left over after commenting out rotation
    ...
    if (rotationButtonTop == true || rotationButtonMid == true || rotationButtonLow == true
      ...
      if (Math.abs(Xbox.getY(Hand.kLeft)) > .5 || Math.abs(Xbox.getX(Hand.kLeft)) > .5) {
        letsRoll.driveCartesian(Xbox.getX(Hand.kLeft), Xbox.getY(Hand.kLeft) * -1, Xbox.getX(Hand.kRight), 0.0);
          ...
          || Math.abs(Xbox.getX(Hand.kRight)) < .5 && (Math.abs(Xbox.getY(Hand.kRight)) > .15)) {
        letsRoll.driveCartesian(Xbox.getX(Hand.kLeft) * .5, Xbox.getY(Hand.kLeft) * -.5, Xbox.getX(Hand.kRight) * .5,
            0.0);
      } else {
        letsRoll.driveCartesian(Xbox.getX(Hand.kLeft) * .5, Xbox.getY(Hand.kLeft) * -.5, Xbox.getX(Hand.kRight) * .5,
            0.0);
  ...
  private void manualOverride() {
    ...
    if (Xbox.getAButton() && limitFront.get() == false) {
      Spike.set(Relay.Value.kForward);  // If a is pressed and the claw isn't forward, move claw forward
